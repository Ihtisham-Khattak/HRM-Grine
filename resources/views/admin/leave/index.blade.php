                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="d-flex justify-content-center ">
                            {{-- <a href="{{ route('leaves.edit', $leave->id) }}" class="btn btn-outline-success btn-sm
                            mx-2"> --}}
                            <a href="{{ Auth::user()->role->slug === 'super-admin' ? route('leaves.edit', $leave->id) : (Auth::user()->role->slug === 'administrator' ? route('admin.leaves.edit', $leave->id) : route('hr.leaves.edit', $leave->id) ) }}"
                                class="btn btn-outline-success btn-sm mx-2">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form
                                action="{{ Auth::user()->role->slug === 'super-admin' ? route('leaves.destroy', $leave->id) : (Auth::user()->role->slug === 'administrator' ? route('admin.leaves.destroy', $leave->id) : route('hr.leaves.destroy', $leave->id) ) }}"
                                method="post">
                                @csrf
                                @method('delete')
